...
def greedy(model, x, tgt_vocab, max_trg_len=15):
    ...
    i2w = {key: value for value, key in tgt_vocab.items()}
    ...
    restore_x = [[i2w[idx.detach().cpu().tolist()] for idx in x[i]] for i in range(len(x))]
    restore_x = [' '.join(line) for line in restore_x]
    restore_y = [[i2w[idx.detach().cpu().tolist()] for idx in y[i]] for i in range(len(y))]
    restore_y = [' '.join(line) for line in restore_y]
