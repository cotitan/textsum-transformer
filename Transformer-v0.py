...
class ScaledDotAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        ...
        :params k: key, size=[batch, n_head, k_len, d_k]
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        ...
        # context = context.transpose(1, 2).contiguous()  # batch, q_len, n_head, d_v
        # context = context.view(context.shape[0], context.shape[1], -1)
        context = torch.cat([context[:, i, :, :] for i in range(context.shape[1])], dim=-1)
