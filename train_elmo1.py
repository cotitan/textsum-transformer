...
import torch
import argparse
from Transformer import ElmoTransformer1
...
parser.add_argument('--ckpt_file', type=str, default='./models/params_elmo1_0.pkl')
args = parser.parse_args()

logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(levelname)s - %(message)s',
    filename='log/train_elmo1.log',
    filemode='w'
)
...
def train(train_x, train_y, y_ids, valid_x, valid_y, valid_y_ids, model,
    ...
    for epoch in range(epoch, n_epochs):
        ...
        torch.save(save_state, os.path.join(model_dir, 'params_elmo1_%d.pkl' % epoch))
...
def main():
    ...
    model = ElmoTransformer1(max_src_len, len(vocab), 1, 4, 64, 64, 256, 1024).cuda()
