...
class EncoderLayer(nn.Module):
    def __init__(self, n_head, d_k, d_v, d_model, d_inner):
        super(EncoderLayer, self).__init__()
        self.multi_attn = MultiheadAttention(n_head, d_k, d_v, d_model)
        self.poswise_ffn = PositionWiseFeedForwardNet(d_model, d_inner)
...
class Encoder(nn.Module):
    def __init__(self, n_layer, n_head, d_k, d_v, d_model, d_inner):
        super(Encoder, self).__init__()
        self.layers = nn.ModuleList(
            [EncoderLayer(n_head, d_k, d_v, d_model, d_inner) for _ in range(n_layer)]
        )
...
class DecoderLayer(nn.Module):
    def __init__(self, n_head, d_k, d_v, d_model, d_inner):
        ...
        self.multi_attn_masked = MultiheadAttention(n_head, d_k, d_v, d_model)
        self.multi_attn = MultiheadAttention(n_head, d_k, d_v, d_model)
        self.poswise_ffn = PositionWiseFeedForwardNet(d_model, d_inner)
...
class Decoder(nn.Module):
    def __init__(self, n_layers, n_head, d_k, d_v, d_model, d_inner):
        ...
        self.layers = nn.ModuleList(
            [DecoderLayer(n_head, d_k, d_v, d_model, d_inner) for _ in range(n_layers)]
        )
...
class Transformer(nn.Module):
    def __init__(self, n_src_vocab, n_tgt_vocab, max_src_len, max_tgt_len,
        ...
        self.src_embedding = Embedding(n_src_vocab, d_model, max_src_len)
        self.tgt_embedding = Embedding(n_tgt_vocab, d_model, max_tgt_len)

        self.encoder = Encoder(n_layer, n_head, d_k, d_v, d_model, d_inner)
        self.decoder = Decoder(n_layer, n_head, d_k, d_v, d_model, d_inner)
    ...
    def forward(self, src_seq, tgt_seq):
        ...
        src_embeds = self.src_embedding(src_seq)
        tgt_embeds = self.tgt_embedding(tgt_seq)
...
class TransformerShareEmbedding(nn.Module):
    def __init__(self, n_vocab, max_seq_len, n_layer, n_head, d_model, d_k, d_v, d_inner):
